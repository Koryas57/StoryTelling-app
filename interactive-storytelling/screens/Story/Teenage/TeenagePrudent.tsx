...
import teenagePrudentData from '../../../data/TeenagePrudentData';
...
type TeenagePrudentProps = NativeStackScreenProps<
    RootStackParamList,
    'TeenagePrudent'
>;
...
const TeenagePrudent: React.FC<TeenagePrudentProps> = ({
    route,
    navigation,
}) => {
    ...
    useEffect(() => {
        if (currentDay <= 7) {
            const dayData = teenagePrudentData[currentDay];
    ...
    const playSoundForDay = async (day: number) => {
        ...
        const daySound = teenagePrudentData[day]?.sound;
        if (daySound) {
...
Recommencez, explorez vos vérités, et écrivez une histoire plus lumineuse, une histoire qui VOUS ressemble. 🔮`,
            ...
            const selectedConsequence = teenagePrudentData[currentDay]?.consequences?.[consequenceKey];
        ...
        >
            <View style={styles.container}>
                <ScrollView ref={scrollViewRef} contentContainerStyle={styles.scrollContainer}>
                    ...
                    <Text style={styles.hudTitle}>{teenagePrudentData[currentDay]?.title}</Text>
                    {teenagePrudentData[currentDay]?.image && (
                        <Image
                            source={teenagePrudentData[currentDay].image}
                ...
                {showTransition && (
                    currentDay <= 6 ? (
                        <Modal visible={showTransition} animationType="fade" statusBarTranslucent={true}>
                            <ImageBackground
                                source={teenagePrudentData[currentDay + 1]?.image}
