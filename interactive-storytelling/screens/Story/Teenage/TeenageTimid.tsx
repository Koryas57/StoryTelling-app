...
const TeenageTimid: React.FC<TeenageTimidProps> = ({
    route,
    navigation,
}) => {
    ...
    useEffect(() => {
        if (currentDay <= 7) {
            const dayData = teenageTimidData[currentDay];
    ...
    const playSoundForDay = async (day: number) => {
        ...
        const daySound = teenageTimidData[day]?.sound;
        if (daySound) {
        ...
        >
            <View style={styles.container}>
                <ScrollView ref={scrollViewRef} contentContainerStyle={styles.scrollContainer}>
                    ...
                    <Text style={styles.hudTitle}>{teenageTimidData[currentDay]?.title}</Text>
                    {teenageTimidData[currentDay]?.image && (
                        <Image
                            source={teenageTimidData[currentDay].image}
                ...
                {showTransition && (
                    currentDay <= 6 ? (
                        <Modal visible={showTransition} animationType="fade" statusBarTranslucent={true}>
                            <ImageBackground
                                source={teenageTimidData[currentDay + 1]?.image}
